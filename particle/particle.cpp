...
Particle::Particle(const char* name, Momentum momentum)
    : index_(findParticle(name)), momentum_(momentum) {
  if (findParticle(name) == -1)
    std::cout << "Invalid name. Initialized particle as default particle with "
                 "index -1.\n";
}
...
void Particle::addParticleType(const char* name, double mass, int charge,
  ...
  if (nParticleTypes_ < maxNumParticleTypes_) {
    if (index != -1)
      std::cout << "Particle with name \"" << name
                << "\" has already been added at index " << index << ".\n";
    else {
      if (width) {
        particleTypes_[nParticleTypes_] =
            new ResonanceType(name, mass, charge, width);
        nParticleTypes_++;
        std::cout << "Added resonance type with name " << name << " at index "
                  << nParticleTypes_ - 1 << ".\n";
      } else {
        particleTypes_[nParticleTypes_] = new ParticleType(name, mass, charge);
        nParticleTypes_++;
        std::cout << "Added particle type with name " << name << " at index "
                  << nParticleTypes_ - 1 << ".\n";
      }
    }
  } else
...
int Particle::decayToBody(Particle& dau1, Particle& dau2) const {
  ...
  dau1.setMomentum({pout * sin(theta) * cos(phi), pout * sin(theta) * sin(phi),
                    pout * cos(theta)});
  dau2.setMomentum({-pout * sin(theta) * cos(phi),
