...
void analyze() {
	...
	if (pTypesH->GetEntries() == nEvents*nParticles    &&
	    ...
	    tMomentumH->GetEntries() == nEvents*nParticles &&
			...
			std::cout << "Generated particle data histograms have the expected number of entries." << std::endl;
	else std::cout << "WARNING! Generated particle data histograms do not have the expected number of entries!";
	std::cout << std::endl;

	const int expPTypesN = 7; // expected particle types number
	if (expPTypesN != pTypesH->GetNbinsX())
		std::cout << "WARNING! The expected number of particles is different from the pTypesH bin number!";
	...
	for(int i{0}; i < expPTypesN; ++i){
		std::cout << "Percentage for " << particleNames[i] << " particle: " << pTypesPercentages[i] << " +/- "<< pTypesPercErrs[i] << std::endl;
	}
	std::cout << std::endl;
	...
	// thetaH->SetOption("stat=e");

	std::cout << std::endl << "Theta fit results:" << std::endl;
	double probDensityTheta { thetaF->GetParameter(0) / thetaH->GetEntries() * thetaH->GetNbinsX() / M_PI / 2. };
	std::cout << "Theta probability density      =  " << probDensityTheta
	<< " +/- " << thetaF->GetParError(0) / thetaF->GetParameter(0) * probDensityTheta << std::endl;
	...
	// phiH->SetOption("stat=e");

	std::cout << std::endl << "Phi fit results:" << std::endl;
	double probDensityPhi { phiF->GetParameter(0) / phiH->GetEntries() * phiH->GetNbinsX() / M_PI };
	std::cout << "Phi probability density        =  " << probDensityPhi
	<< " +/- " << phiF->GetParError(0) / phiF->GetParameter(0) * probDensityPhi << std::endl;
	std::cout << "Chisquare/NDF                  =  " << phiF->GetChisquare() / phiF->GetNDF() << std::endl;
	std::cout << "Fit likelyhood                 =  " << phiF->GetProb() << std::endl;
	...
	// momentumH->SetOption("stat=e");

	std::cout << std::endl << "Momentum fit results:" << std::endl;
	std::cout << "Tau value                      =  " <<  momentumF->GetParameter(0) << " +/- "
	<< momentumF->GetParError(0) / momentumF->GetParameter(0) * 1 / momentumF->GetParameter(0) << std::endl;
	std::cout << "Amplitude value                =  " << momentumF->GetParameter(1) << " +/- "
	<< momentumF->GetParError(1) << std::endl;
	std::cout << "Chisquare/NDF                  =  " << momentumF->GetChisquare() / momentumF->GetNDF() << std::endl;
	...
	std::cout << std::endl << "All particles invariant mass subtraction fit results:" << std::endl;
	...
	std::cout << "Chisquare/NDF                  =  " << invMassF->GetChisquare() / invMassF->GetNDF() << std::endl;
	std::cout << "Fit likelyhood                 =  " << invMassF->GetProb() << std::endl;
	...
	std::cout << std::endl << "Pi and K particles invariant mass subtraction fit results:" << std::endl;
	std::cout << "Mean value (k* invariant mass) =  " << invMassPKF->GetParameter(1) << " +/- "
	<< invMassPKF->GetParError(1) << std::endl;
	std::cout << "Standard deviation (k* width)  =  " << invMassPKF->GetParameter(2) << " +/- "
	<< invMassPKF->GetParError(2) << std::endl;
	std::cout << "Amplitude                      =  " << invMassPKF->GetParameter(0) << " +/- "
	<< invMassPKF->GetParError(0) << std::endl;
	std::cout << "Chisquare/NDF                  =  " << invMassPKF->GetChisquare() / invMassPKF->GetNDF() << std::endl;
	std::cout << "Fit likelyhood                 =  " << invMassPKF->GetProb() << std::endl;
	...
	std::cout << "Mean value (k* invariant mass) =  " << invMassKStarF->GetParameter(1) << " +/- "
	<< invMassKStarF->GetParError(1) << std::endl;
	std::cout << "Standard deviation (k* width)  =  " << invMassKStarF->GetParameter(2) << " +/- "
	<< invMassKStarF->GetParError(2) << std::endl;
	std::cout << "Amplitude                      =  " << invMassKStarF->GetParameter(0) << " +/- "
	<< invMassKStarF->GetParError(0) << std::endl;
	std::cout << "Chisquare/NDF                  =  " << invMassKStarF->GetChisquare() / invMassKStarF->GetNDF() << std::endl;
	std::cout << "Fit likelyhood                 =  " << invMassKStarF->GetProb() << std::endl << std::endl;

	TFile* output = new TFile("output/processedData.root", "RECREATE");
	
	const char* pTypes[expPTypesN] { "#pi+", "#pi-", "K+", "K-", "P+", "P-", "K*" };
	gStyle->SetTextFont(42);
	for (int i {1}; i <= expPTypesN; ++i) pTypesH->GetXaxis()->SetBinLabel(i, pTypes[i - 1]);

	TCanvas* particlesCvs = new TCanvas("particlesCanvas",
	"Dati su tipi e informazioni dinamiche delle particelle",
	1600, 1200);

	TH1F* histograms[7] {pTypesH, thetaH, phiH, momentumH, invMassOppSgnH, invMassPKOppH, invMassKStarH};
	TF1* functions[6] {thetaF, phiF, momentumF, invMassF, invMassPKF, invMassKStarF};
	...
	for (TH1F* histo: histograms) {
		...
		histo->SetName("Risultati"); // terrible but only quasi-decent way to set the stats box title
		if (i > 0) {
			histo->GetFunction(functions[i-1]->GetName())->SetLineColor(kGreen + 3);
		}
		++i;
	}
	}
	...
	TCanvas* invMassCvs = new TCanvas("invMassKStarCvs",
	"Distribuzione della massa invariante delle particelle",
	1800, 600);
