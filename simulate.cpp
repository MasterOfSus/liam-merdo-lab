...
void simulate() {
  ...
  invMassEqSgnH->Sumw2();
  TH1F* invMassPKOppH =
      new TH1F("invMassPKOppH", "Invariant mass, opp. sign p and k", 1000, 0., histoExtInvMassPK);
  invMassPKOppH->Sumw2();
  TH1F* invMassPKEqH =
      new TH1F("invMassPKEqH", "Invariant mass, equal sign p and k", 1000, 0., histoExtInvMassPK);
  invMassPKEqH->Sumw2();
  ...
  int tailIndex{100};  // index for first empty space in the eventParticles
                       // array after the 100th spot
  ...
  for (int i{}; i < nEvents; ++i) {
    ...
    for (int j{}; j < nGens; ++j) {
      ...
      pNorm = gRandom->Exp(1.);
      eventParticles[j].setMomentum({sin(phi) * cos(theta) * pNorm,
                                     sin(phi) * sin(theta) * pNorm,
                                     cos(phi) * pNorm});

      control = gRandom->Uniform(0., 1.);
      if (control < 0.4)
        eventParticles[j].setIndex(piPlusI);  // pi+, 40%
      else if (control < 0.8)
        eventParticles[j].setIndex(piMinI);  // pi-, 40%
      else if (control < 0.85)
        eventParticles[j].setIndex(kPlusI);  // k+, 5%
      else if (control < 0.9)
        eventParticles[j].setIndex(kMinI);   // k-, 5%
      else if (control < 0.945)
        eventParticles[j].setIndex(pPlusI);  // P+, 4.5%
      else if (control < 0.99)
        eventParticles[j].setIndex(pMinI);   // P-, 4.5%
      else {
